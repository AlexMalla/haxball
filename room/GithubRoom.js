...
function helpFun() { // !help
	room.sendChat('Comandi disponibili: "p", "!p" , "!stats Nickname", "!ranking", "!resetstats", "!adminhelp", "!gkhelp", "!rankhelp"');
}
 
function adminHelpFun() {
	room.sendChat('Comandi disponibili: "!mute Player", "!unmute Player", ' +
	'"!clearbans", "!rr", "!swap" (to switch reds and blues). You need to be admin.')
}
 
 
function gkHelpFun() { // !gkhelp
	room.sendChat('Il giocatore più arretrato della squadra verrà settato come portiere! (scrivi "!gk" se il bot si sbaglia).')
}
function rankHelpFun() { // !gkhelp
	room.sendChat("Ottieni punti facendo cose buone! goal: 5 pts, assist: 3 pts, vittoria: 3 pts, cs: 6 pts, sconfitta: -7 pts, autogol: -4 pts.")
}
...
function sendStats(name){
	...
	+ ", autogol: " + ps[4] + ", cs: " + ps[5] + ", vittorie: " + ps[2] + ", sconfitte: " + ps[3] +
	" punti: " + rankingCalc(name));
}
...
room.onPlayerJoin = function(player) {
	...
	room.sendChat("Ciao signor coglione, ah no scusa, signor " + player.name + " ! Scrivi !help per aiuto, !stats per vedere quanto sei scarso." )
}
